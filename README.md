...
`uidetox redesign [target] --variants 3` consumes that map and writes `.uidetox/redesigns.json`. Stale maps are refreshed automatically when mapped source files are added, changed, deleted, or the extractor changes. Proposals change page topology, navigation, component ownership, action placement, interaction model, responsive composition, and density before suggesting source edits. Reliable lifecycle evidence also produces owner-level plans for loading, empty, error, success, disabled, and first-run experiences; unknown, invalid, or contradictory evidence becomes a blocker instead of a guessed fix. Pairwise structural distance prevents three cosmetic variants of the same layout.

`uidetox compare` presents every proposal across the same seven structural dimensions and shows pairwise distance. `uidetox prototype <proposal-id>` writes an isolated, agent-ready brief under `.uidetox/prototypes/`; mapped source evidence is explicitly quarantined as untrusted data, and large experience-state matrices retain representative owner and operation evidence without dropping a missing state category or read/mutation mode.
...
| `uidetox redesign [target]` | Generate 1–5 structurally divergent, contract-preserving redesign plans with fail-closed experience-state coverage (`--variants`, `--refresh-map`, `--map-file`, `--output`, `--json`). |
...
| `uidetox prototype <proposal-id>` | Create a disposable implementation brief with contracts, migration steps, representative experience-state evidence, acceptance checks, and untrusted-evidence isolation (`--file`, `--output`, `--stdout`). |
