...
def run(args: argparse.Namespace):
    ...
    if profile.linter:
        print(
            f"  Linter          : {profile.linter.name} ({profile.linter.config_file})"
        )
    ...
    if profile.formatter:
        print(
            f"  Formatter       : {profile.formatter.name} ({profile.formatter.config_file})"
        )
    ...
    for f in getattr(profile, "frontend", []):
