...
def run(args: argparse.Namespace):
    ...
    if not args.note or not args.note.strip():
        print(
            "Error: --note cannot be empty. Provide a brief description of the fix.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not skip_verify:
        ...
        if not _run_verification(config):
            print(
                "❌ Verification failed. Build is broken. Fix it before resolving.",
                file=sys.stderr,
            )
            sys.exit(1)
        print()
    ...
    if success:
        ...
        if config.get("auto_commit", False):
            ...
            sibling_untracked_paths = {
                path
                for path in untracked_paths
                if Path(path).parent == issue_parent_dir
            }
            ...
            if unexpected_tracked_changes or unexpected_untracked_changes:
                print(
                    "   ⚠️  Skipped git auto-commit because changes exist outside the resolved file."
                )
            else:
                try:
                    ...
                    print("   ⚠️  Warning: Git auto-commit failed. Is this a git repo?")
                except FileNotFoundError:
                    print("   ⚠️  Warning: git command not found. Skipping auto-commit.")
        ...
        save_session(
            phase="fixing",
            last_command="resolve",
            last_component=issue.get("file", ""),
            issues_fixed=1,
            context=args.note,
        )

        # Persist fix outcome for future sub-agent context injection
        try:
            record_fix_outcome(
