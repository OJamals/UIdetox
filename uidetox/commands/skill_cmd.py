...
def run(args: argparse.Namespace):
    ...
    print(f"Skill '{skill_name}' context loaded. Apply the rules above to '{target}'.")
    print("After applying fixes:")
    print("  1. Run `uidetox check --fix` to verify build integrity")
    print("  2. Run `uidetox batch-resolve <IDs> --note '...'` for fixed issues")
    print("  3. Continue the loop — run `uidetox next` for the next batch")
    print()
    print("Track learnings:  uidetox memory pattern \"Discovered that...\"")
    print("Check progress:   uidetox status")
    print()
    print("DO NOT STOP. Continue executing the autonomous loop.")
