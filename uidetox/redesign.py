...
_OPERATION_OBLIGATION_ACTIONS = {
    ...
    "auth-required": "disable until authenticated; expose the documented recovery path",
    ...
    "forbidden": "keep unavailable; explain the documented authorization boundary",
    "idempotency": "apply only the server-defined idempotency scope, retention, and replay semantics",
    "optimistic-rollback": "reconcile against the response; restore the prior value on failure",
    "partial-success": "separate succeeded items from operation-scoped failures",
    "rate-limit": "show the retry boundary; auto-retry only when separately proven",
